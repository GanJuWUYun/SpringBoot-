            ...
            <div class="layui-card">
                <div class="layui-card-body ">
                    <div class="layui-input-inline layui-show-xs-block">
                                ...
                                <#if onThirdMenu.parentId == currentMenu.id>
                                    <a onclick="${onThirdMenu.url!""}">
                                        <#if onThirdMenu.url == "openEditState();">
                                            <button class="layui-btn layui-btn-warm layui-btn-xs"><i
                                                        class="icon iconfont">${onThirdMenu.icon!""}</i>&nbsp;${onThirdMenu.name!""}
                                            </button>
                                        <#elseif onThirdMenu.url == "deleteOrder();">
                                            <button class="layui-btn-danger layui-btn layui-btn-xs"><i
                                                        class="icon iconfont">${onThirdMenu.icon!""}</i>&nbsp;${onThirdMenu.name!""}
                                            </button>
                                        <#else>
                        ...
                        &nbsp;&nbsp;
                        <div class="layui-input-inline layui-show-xs-block">
                            <input class="layui-input" onkeyup="value=value.replace(/^0(0+)|[^\d]+/g,'')"
                                   placeholder="订单流水号" id="search-name" value="${content!""}">
                        </div>
                        <button class="layui-btn" onclick="search();"><i class="icon iconfont">&#xe6ac;</i>&nbsp;搜索
                        </button>
                    </div>
                            ...
                            <#list PageInfo.list as order>
                                <tr>
                                    <td style="text-align:center">
                                        <input type="checkbox" name="" id="${order.id!""}" method="checkedId"
                                               lay-skin="primary">
                                    </td>
                ...
                <div class="layui-card-body ">
                    <div class="page">
                        <div>
                            ...
                            <#else>
                                <a class="prev"
                                   href="/admin/order/index?id=${currentMenu.id!""}&pageNum=${PageInfo.prePage!""}&content=${content!""}">&lt;&lt;</a>
                            </#if>
                            <#list PageInfo.pageNum-2..PageInfo.pageNum-1 as i>
                                <#if i gte 1>
                                    <a class="num"
                                       href="/admin/order/index?id=${currentMenu.id!""}&pageNum=${i!""}&content=${content!""}">${i!""}</a>
                                </#if>
                            </#list>
                            ...
                            <#else>
                                <a class="next"
                                   href="/admin/order/index?id=${currentMenu.id!""}&pageNum=${PageInfo.nextPage!""}&content=${content!""}">&gt;&gt;</a>
                            </#if>
                        </div>
                    </div>
                </div>
